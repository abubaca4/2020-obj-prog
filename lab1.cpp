...
Money Money::operator+(const Money& rv) const
{
    ...
    if (znak == 1)
    {
        return Money((this->rub < 0 ? -1 : 1) * abs(abs(this->rub) + abs(rv.get_rub()) + (this->cop + rv.get_cop()) / 100), (this->cop + rv.get_cop()) % 100);
    }
    else
    {
        ...
        return Money((big->get_rub() < 0 ? -1 : 1)*(abs(big->get_rub()) - (abs(small->get_rub()) + (small->get_cop() > big->get_cop() ? 1 : 0))), big->get_cop() - small->get_cop() + (small->get_cop() > big->get_cop() ? 100 : 0));
    }
}
...
{
    return Money((count >= 0 ? 1 : -1 * this->rub >= 0 ? 1 : -1)*(abs((this->cop * count) / 100) + abs(count*this->rub)), abs((this->cop * count) % 100));
}

Money Money::operator/(const int64_t count) const
{
    return Money(this->rub / count, (abs(this->rub % count) * 10 + this->cop) / abs(count));
}
...
class Person
{
    ...
    Money pouch;
public:
    Person(string first_name, string second_name): name(first_name), last_name(second_name) {}
